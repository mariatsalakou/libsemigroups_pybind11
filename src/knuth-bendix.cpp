...
namespace libsemigroups {
    ...
    py::enum_<fpsemigroup::KnuthBendix::policy::overlap>(kb, "overlap")
        .value("ABC", fpsemigroup::KnuthBendix::policy::overlap::ABC)
        .value("AB_BC", fpsemigroup::KnuthBendix::policy::overlap::AB_BC)
        .value("MAX_AB_BC", fpsemigroup::KnuthBendix::policy::overlap::MAX_AB_BC);
	...
	.def("normal_form", 
	     py::overload_cast<word_type const &>( &libsemigroups::fpsemigroup::KnuthBendix::normal_form), py::arg("w"),
             R"pbdoc(
	...
	.def("validate_word",
             py::overload_cast<std::string const &>(
             &libsemigroups::fpsemigroup::KnuthBendix::validate_word, py::const_),
             py::arg("w"),
	     R"pbdoc(
            Validates a word.
	...
	.def("run_for",
	     (void (libsemigroups::fpsemigroup::KnuthBendix::*)(
	...
	.def("rules_iterator",
             ...
             return py::make_iterator(kb.cbegin_rules(), 
 			                kb.cend_rules());
	       },
             R"pbdoc(
	...
	.def("normal_forms",
	     ...
	     return py::make_iterator(kb.cbegin_normal_forms(mn, mx), kb.cend_normal_forms());
		        },
	  R"pbdoc(
          Returns an iterator to the generating pairs of the congruence.
          )pbdoc")
	.def("normal_forms_alphabet",
	  [](fpsemigroup::KnuthBendix &kb, std::string const &lphbt, size_t const mn, size_t const mx) {
	  return py::make_iterator(kb.cbegin_normal_forms(lphbt,mn, mx), kb.cend_normal_forms());
					                                                                       },
	  R"pbdoc(
          Returns an iterator to the generating pairs of the congruence.
          )pbdoc")
	...
	.def("add_rules",
	     py::overload_cast<FroidurePinBase &>(&libsemigroups::fpsemigroup::KnuthBendix::add_rules),
	     py::arg("S"),
	     R"pbdoc(
    ...
    .def(py::init<std::shared_ptr<FroidurePinBase>>())
     ...
     .def("cbegin_normal_forms",
  	    py::overload_cast<std::string const &, size_t const, size_t const>(
    ...
    length is in the given range using the specified alphabet.
           ...
           :Parameters: - **lphbt** (??) - the alphabet to use for the normal
    forms
                        - **min** (??) - the minimum length of a normal form
                        - **max** (??) - one larger than the maximum length of a
    normal form.

           :Returns: A value of type const_normal_form_iterator.
           )pbdoc")
  
    .def("cbegin_normal_forms",
         py::overload_cast<size_t const, size_t const>(
             &libsemigroups::fpsemigroup::KnuthBendix::cbegin_normal_forms),
         py::arg("min"),
         py::arg("max"),
         R"pbdoc(
    ...
    KnuthBendix::alphabet.
           ...
           :Parameters: - **min** (??) - the minimum length of a normal form
                        - **max** (??) - one larger than the maximum length of a
    normal form.
    ...
    .def("cend_normal_forms",
         ...
         R"pbdoc(
           Returns a forward iterator pointing to one after the last normal
    form.
